...
ssh -o ProxyCommand="ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -W %h:%p ubuntu@$bastion_ip" ubuntu@$cp1_ip "sudo cat /etc/kubernetes/admin.conf" > ~/.kube/config
